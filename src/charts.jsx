...
// ───────────────────────────────────────────────────────────────────────
// MiniBars — clean little bar trend for KPI / hero slots (replaces the
// sparkline "EKG" look). Bars are baseline-anchored; the most recent bar is
// emphasised so the eye lands on "now".
// ───────────────────────────────────────────────────────────────────────
function MiniBars({ data, color = 'currentColor', height = 28, highlightLast = true }) {
  ...
  return (
    <svg viewBox={`0 0 100 ${height}`} preserveAspectRatio="none" style={{ width: '100%', height }}>
      {vals.map((v, i) => {
        const h = Math.max(1.2, ((v - min) / range) * (height - 2));
        const last = i === n - 1;
        return (
          <rect key={i} x={i * bw + bw * 0.14} y={height - h}
            width={bw * 0.72} height={h} rx={bw > 6 ? 0.8 : 0}
            fill={color} opacity={highlightLast && !last ? 0.4 : 0.92} />
        );
      })}
    </svg>
  );
  ...
  return (
    <div className="chart" ref={wrapRef}>
      <svg viewBox={`0 0 ${W} ${H}`} onMouseMove={handleMove} onMouseLeave={handleLeave}>
        ...
        {/* X labels — sparse. The final label is always shown; skip any stepped
            label that would crowd it so the dates never overlap at the right edge.
            Anchor the last label to its end and the first to its start so neither
            spills past the plot. */}
        {showAxis && labels.map((l, i) => {
          const step = Math.max(1, Math.ceil(n / 8));
          const isLast = i === n - 1;
          if (!isLast) {
            if (i % step !== 0) return null;
            if (n - 1 - i < step * 0.6) return null;
          }
          const anchor = isLast ? 'end' : (i === 0 ? 'start' : 'middle');
          return (
            <text key={i} x={xs(i)} y={H - 8} textAnchor={anchor} fontSize="10" fontFamily="var(--mono)" fill="var(--ink-4)" style={{ letterSpacing: '0.04em' }}>
              {l}
            </text>
          );
        })}
      ...
      {/* Tooltip — clamp horizontally so it never spills past the plot edges,
          and hang it just below the top so it can't clip against the chart top.
          Near the right edge the box right-aligns to the point; near the left
          it left-aligns; otherwise it stays centered. */}
      {hover !== null && (() => {
        const left = (xs(hover) / W) * 100;
        const tx = left > 78 ? '-100%' : left < 22 ? '0%' : '-50%';
        return (
          <div className="chart__tip" style={{ left: `${left}%`, top: `${(padT / H) * 100}%`, transform: `translate(${tx}, 0) translateY(6px)` }}>
            <em>{labels[hover]}</em>
            {series.map((s, i) => (
              <div key={i}>
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        {centerLabel && (
          <>
            ...
            {centerSub && (
              <text x={c} y={c + 16} textAnchor="middle" fontFamily="var(--mono)" fontSize="9.5" fill="var(--ink-4)" letterSpacing="0.08em" style={{ textTransform: 'uppercase' }}>
                {centerSub}
              </text>
            )}
          </>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      {steps.map((s, i) => {
        ...
        return (
          <div key={i} style={{ borderBottom: i < steps.length - 1 ? '1px solid var(--rule)' : 0, padding: '10px 0', display: 'grid', gridTemplateColumns: '130px 1fr 72px 58px', gap: 14, alignItems: 'center' }}>
            <div style={{ fontFamily: 'var(--serif)', fontSize: 14, color: 'var(--ink)', fontWeight: 500 }}>{s.label}</div>
            <div style={{ height: 18, background: 'var(--rule)', borderRadius: 2, overflow: 'hidden', position: 'relative' }}>
  ...
  return (
    <div style={{ display: 'flex', gap: 1, height, width: '100%' }}>
      {data.map((d, i) => {
        const v = d.value;
        let bg = 'var(--rule)';
        if (v > thresholdHi) bg = `rgba(218,32,55, ${Math.min(1, 0.3 + Math.abs(v) * 4)})`;
        else if (v < thresholdLo) bg = `rgba(198,138,46, ${Math.min(1, 0.3 + Math.abs(v) * 4)})`;
        else bg = `rgba(47,143,107, ${0.18 + Math.abs(v) * 2})`;
        return (
...
Object.assign(window, { Sparkline, MiniBars, LineChart, HorizontalBars, Donut, Funnel, HeatStrip });
