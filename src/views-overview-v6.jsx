...
function OverviewView({
  ...
  // "Available" palette (hidden modules/KPIs/rows) is summoned from the
  // edit bar rather than floating permanently over the page.
  const [paletteOpen, setPaletteOpen] = React.useState(false);
  React.useEffect(() => { if (!editing) setPaletteOpen(false); }, [editing]);
  ...
  const heroHidden = !!safeLayout.heroHidden;
  const attentionHidden = !!safeLayout.attentionHidden;
  const hideHero = () => updateLayout({ heroHidden: true });
  const showHero = () => updateLayout({ heroHidden: false });
  const hideAttention = () => updateLayout({ attentionHidden: true });
  const showAttention = () => updateLayout({ attentionHidden: false });
  ...
  const opSpend = V.complete(D.finance.operatingSpend);
  const donTotal = V.complete(D.donations.total);
  const trendMonths = V.complete(months);
        ...
        <table className={'tbl' + (editing ? ' tbl--editing' : '')}>
          <thead>
            <tr>
              ...
              <th>Department</th>
              <th>Lead</th>
              <th style={{ width: 110 }}>Spend vs budget</th>
            ...
            {deptsVisible.map((d, i) => {
              ...
              return (
                <tr key={d.id} className={rowCls.trim() || undefined} {...dragProps} onClick={editing ? undefined : () => onDrill({ kind: 'department', deptId: d.id })}>
                  ...
                  <td>
                    {/* Department's ACTUAL spend over the last 12 complete months
                        (solid line) against its monthly budget (dashed line). */}
                    {(() => {
                      const vals = window.PRTS_VIEW.complete(d.series).slice(-12);
                      const mb = d.monthlyBudget;
                      const peak = Math.max(mb, ...vals) * 1.08;
                      const floor = Math.min(mb, ...vals) * 0.92;
                      const range = (peak - floor) || 1;
                      const x = (i) => (i / (vals.length - 1)) * 98 + 1;
                      const y = (v) => 22 - ((v - floor) / range) * 20;
                      const pts = vals.map((v, i) => `${x(i).toFixed(1)},${y(v).toFixed(1)}`).join(' ');
                      return (
                        <svg viewBox="0 0 100 24" width="100" height="24" style={{ display: 'block' }}>
                          <line x1="0" x2="100" y1={y(mb)} y2={y(mb)} stroke="var(--ink-4)" strokeWidth="1" strokeDasharray="3 2" vectorEffect="non-scaling-stroke" />
                          <polyline points={pts} fill="none" stroke={col} strokeWidth="1.6" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke" />
                          <circle cx={x(vals.length - 1)} cy={y(vals[vals.length - 1])} r="1.8" fill={col} />
                        </svg>
                      );
                    })()}
                  </td>
    ...
    <>
      ...
      {editing && (
        <CustomizeBar
          ...
          editStyle={editStyle}
          userName={U.short}
          availableCount={hiddenSections.length + (heroHidden ? 1 : 0) + (attentionHidden ? 1 : 0) + hiddenKpis.length + deptsHiddenList.length + giftsHiddenList.length + (signals.filter(s => (safeLayout.signalHidden || []).includes(s.id)).length)}
      ...
      <div className="pagehead-row">
        <PageHead
          ...
          eyebrow={<><strong>{greeting}, {U.short}.</strong> &nbsp;·&nbsp; Today, {todayLong}</>}
          title="Overview"
          meta={[
      ...
      {/* Hero row (agenda/today calendar removed) — hero metric + attention,
          each removable in edit mode via its corner minus button. */}
      {(!heroHidden || !attentionHidden) && (
        <div className={'hero-row hero-row--no-agenda' + ((heroHidden || attentionHidden) ? ' hero-row--solo' : '')}>
          {!heroHidden && (
            <div className="hero-frame" style={{ position: 'relative' }}>
              {editing && (
                <button type="button" className="hero-frame__hide"
                  onClick={(e) => { e.stopPropagation(); hideHero(); }}
                  title="Remove the hero metric">
          ...
          {!attentionHidden && (
            <div className="attn-frame" style={{ position: 'relative' }}>
              {editing && (
                <button type="button" className="attn-frame__hide"
                  onClick={(e) => { e.stopPropagation(); hideAttention(); }}
                  title="Remove the attention panel">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="5" y1="12" x2="19" y2="12"/></svg>
                </button>
              )}
              <HeroSummary
      ...
      <ModulePalette
        ...
        attentionHidden={attentionHidden}
