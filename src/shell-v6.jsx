...
function Masthead({ chapter, onNav, badges = {}, onCmdK, layout = 'bauhaus', onLayoutChange, theme = 'light', onThemeChange, user, onUserChange, onExport, onShare, onRefresh, refreshing = false, syncTime = '04:12 GMT', navEditing = false, navOrder, onNavOrderChange }) {
  ...
  // Click anywhere on the dashboard (cards, header, empty background) to
  // dismiss the sidebar — only the sidebar itself, the hamburger, and any
  // floating overlay panels are exempt. Only armed while the sidebar is open.
  React.useEffect(() => {
    ...
    <>
      ...
      <div className="topbar">
        <div className="topbar__inner">
          ...
          <span className={'topbar__sync' + (refreshing ? ' topbar__sync--busy' : '')}><i />{refreshing ? 'Syncing…' : 'Synced ' + syncTime}</span>
          <button
