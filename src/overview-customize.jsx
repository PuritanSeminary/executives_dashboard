...
function migrateLayout(raw) {
  ...
  if (raw.v === LAYOUT_VERSION) return raw;
  if (raw.v === 1) {
    return {
      ...DEFAULT_LAYOUT,
      ...raw,
...
function buildKpiLibrary(D) {
  ...
  const eqPct = D.finance.investments.accounts[0].alloc.eq;
  ...
  return [
    ...
    },
    {
      ...
      props: {
        ...
        caption: <>Greenleaf Trust · <strong>{fmt.pct(eqPct, 0)} equities</strong></>,
        spark: lastN(inv, 36),
        sparkColor: 'var(--ink-3)',
        status: 'pos',
      },
...
function buildHeroLibrary(D) {
  ...
  return [
    ...
    },
    {
      ...
      props: {
        label: 'Investments · total at Greenleaf',
        tag: '+6.2% YTD',
        value: fmt.shortMoney(D.finance.investments.total),
        delta: pctChange(invCurr, invPrior),
        deltaLabel: "vs. May '25",
...
function CustomizeBar({ editing, onToggle, onReset, isCustomized, editStyle, userName }) {
  if (!editing) {
    return (
      <button
        type="button"
    ...
    <div className={'custom-bar custom-bar--' + (editStyle || 'editorial')}>
      <div className="custom-bar__inner">
        ...
        <div className="custom-bar__text">
          <strong>Customizing {userName}'s overview</strong>
          <em>Drag sections to reorder · click eye to hide · drag tiles to swap</em>
        </div>
...
function ModulePalette({
  ...
  onShowSection, onAddKpi, onShowAgenda,
  onShowDept, onShowGift, onShowSignal,
  onClose, kpiBandFull, editStyle,
}) {
  if (!editing) return null;
  const hasHiddenModules = hiddenSections.length > 0 || agendaHidden;
